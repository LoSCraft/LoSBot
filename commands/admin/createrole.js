...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        while (!hex) {
            await Utils.waitForResponse(message.author.id, message.channel).then(async response => {
                await response.delete();
                if (!/#([a-f]|[0-9]){6}/.test(response.content)) return message.channel.send(Embed({ preset: 'error', description: lang.AdminModule.Commands.Createrole.Errors.InvalidHex })).then(m => m.delete(2500));
        ...
        while (!n) {
            await Utils.waitForResponse(message.author.id, message.channel).then(async response => {
                await response.delete();
                if (parseInt(response.content) == NaN) return message.channel.send(Embed({ preset: 'error', description: lang.AdminModule.Commands.Createrole.Errors.InvalidNumber })).then(m => m.delete(2500));
