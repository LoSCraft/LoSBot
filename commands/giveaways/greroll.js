...
module.exports = {
    ...
    run: async (bot, message, args) => {
        if (config.Giveaway_System.Enabled == false) return;
        let role = message.guild.roles.find(r => r.name.toLowerCase() == config.Giveaway_System.GReRoll_Required_Rank.toLowerCase())
        if (!role) return message.channel.send(Embed({ preset: 'console' }));
        if (!Utils.hasPermission(message.member, config.Giveaway_System.GReRoll_Required_Rank)) return message.channel.send(Embed({ preset: 'nopermission' }));
        ...
        if (args.length > 0 && !giveaway) {
            ...
            bot.guilds.first().channels.get(giveaway.channel).fetchMessage(giveaway.messageID).then(async msg => {
                ...
                if (users.length !== undefined && users.length > 1) {
                    ...
                    await end()
                    channel.send(users.map(u => "<@" + u + ">")).then(m => m.delete(2500));
                } else {
                    ...
                    await end()
                    channel.send("<@" + users.id + ">").then(m => m.delete(2500));
                }
    ...
    description: lang.Help.CommandDescriptions.Greroll,
